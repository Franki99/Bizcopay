...
function groupByDay(transactions: Transaction[]): { date: string; volume: number; count: number }[] {
...
function groupByRisk(transactions: Transaction[]) {
  const map = new Map<string, number>()
  for (const tx of transactions) {
    map.set(tx.riskScore, (map.get(tx.riskScore) ?? 0) + 1)
  }
...
export default function AnalyticsPage() {
  ...
  const maxMerchantVolume = merchants[0]?.volume ?? 1
                ...
                <div key={m.name} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(m.volume / maxMerchantVolume) * 100}%` }}
                    />
                  </div>
